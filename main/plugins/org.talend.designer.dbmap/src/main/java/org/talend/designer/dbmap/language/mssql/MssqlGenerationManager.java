...
public class MssqlGenerationManager extends DbGenerationManager {
    ...
    private boolean isConditionChecked(DbMapComponent component, ExternalDbMapTable inputTable) {
        ...
        for (ExternalDbMapEntry dbMapEntry : inputEntries) {
            if (dbMapEntry.isJoin()) {
                if (StringUtils.isNotEmpty(dbMapEntry.getExpression()) && StringUtils.isNotEmpty(dbMapEntry.getOperator())) {
