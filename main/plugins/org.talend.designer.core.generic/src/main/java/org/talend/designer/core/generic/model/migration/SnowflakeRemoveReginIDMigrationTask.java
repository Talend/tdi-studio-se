 ...
 */
public class SnowflakeRemoveReginIDMigrationTask extends AbstractJobMigrationTask {
    ...
    public Date getOrder() {
        GregorianCalendar gc = new GregorianCalendar(2019, 10, 07, 12, 0, 0);
        return gc.getTime();
    }
    ...
    public ExecutionResult execute(Item item) {
        ...
        IComponentConversion changeJDBCDriverJarType = new IComponentConversion() {
            ...
            public void transform(NodeType node) {
                ...
                NamedThing region = newProperties.getProperty("region");
                NamedThing useCustomRegion = newProperties.getProperty("useCustomRegion");
                NamedThing customRegionID = newProperties.getProperty("customRegionID");
                NamedThing account = newProperties.getProperty("account");
                Map<String, String> regionMap = new HashMap<String, String>(){{
                    put("AWS_US_EAST_1", "us-east-1");
                    put("AWS_EU_WEST_1", "eu-west-1");
                    put("AWS_EU_CENTRAL_1", "eu-central-1");
                    put("AWS_AP_SOUTHEAST_2", "ap-southeast-2");
                    put("AZURE_EAST_US_2", "east-us-2.azure");
                }};
                
				String regionID = "";
				boolean useCustom = false;
				if (useCustomRegion != null && useCustomRegion instanceof Property) {
					useCustom = ((Property) useCustomRegion).getValue().equals(true);
					if (useCustom) {
						regionID += ((Property) customRegionID).getValue();
					}
				}
				if (!useCustom && region != null && region instanceof Property) {
					String storedID = regionMap.get(String.valueOf(((Property) region).getValue()));
					if (storedID != null) {
						regionID += storedID;
					}
				}
				if (account != null && account instanceof Property) {
					String storedAccount = String.valueOf(((Property) account).getValue());
					((Property) account).setStoredValue(storedAccount + regionID);
				}
				
				elemParamType.setValue(newProperties.toSerialized());
			}
		};
        String[] componentsName = {"tSnowflakeConnection","tSnowflakeInput","tSnowflakeOutput","tSnowflakeRow","tSnowflakeBulkExec","tSnowflakeOutputBulkExec","tSnowflakeOutputBulk"}; //$NON-NLS-1$
    ...
    private Property getProperty(NamedThing nt) {
