...
public class UpgradePasswordEncrytionAlg4ProjectMigrationTask extends AbstractMigrationTask implements IProjectMigrationTask {
            ...
            for (int i = 0; i < elementParameter.size(); i++) {
                ...
                if (object instanceof ElementParameterType) {
                    ...
                    if ("PASS".equals(name)) { //$NON-NLS-1$
                        try {
                            modified = UpgradePasswordEncryptionAlg4ItemMigrationTask.reencryptValueIfNeeded(parameterType);
                        } catch (Exception e) {
